...
@section('breadcrumb')
    ...
    ], 'active' => __("Setup Mobile Topup Category")])
@endsection
    ...
    <div class="table-area">
        <div class="table-wrapper">
            <div class="table-header">
                <h5 class="title">{{ __("Mobile Topup Categories") }}</h5>
...
@push('script')
    <script>
        ...
        $(".delete-modal-button").click(function(){
            ...
            var actionRoute =  "{{ setRoute('admin.mobile.topup.category.delete') }}";
            var target      = oldData.id;
            var message     = `Are you sure to delete <strong>${oldData.name}</strong> Category?`;
            openDeleteModal(actionRoute,target,message);
        });

        itemSearch($("input[name=category_search]"),$(".category-search-table"),"{{ setRoute('admin.mobile.topup.category.search') }}",1);
    </script>
@endpush
