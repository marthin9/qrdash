...
@push('script')
    <script>
        ...
        $(".delete-modal-button").click(function(){
            ...
            var actionRoute =  "{{ setRoute('admin.bill.pay.category.delete') }}";
            var target      = oldData.id;
            var message     = `Are you sure to delete <strong>${oldData.name}</strong> Category?`;
            openDeleteModal(actionRoute,target,message);
        });

        itemSearch($("input[name=category_search]"),$(".category-search-table"),"{{ setRoute('admin.bill.pay.category.search') }}",1);
    </script>
@endpush
