    ...
    public function toMail($notifiable)
    {
        ...
        if($data->transaction_type == 'bank-transfer') {
            return (new MailMessage())
                        ...
                        ->subject($data->title)
                        ->line("Send Remittance request is rejected by admin successfully, details of Send Remittance:")
                        ->line($data->title)
            ...
            return (new MailMessage())
                    ->greeting("Hello ".$user->fullname." !")
                    ->subject($data->title)
                    ->line("Send Remittance requestis rejected by admin successfully, details of Send Remittance:")
                    ->line($data->title)
                    ->line("Transaction Id: " .$trx_id)
                    ->line("Transaction Type: " . ucwords(str_replace('-', ' ', @$data->transaction_type)))
                    ->line("Request Amount: " . $data->request_amount)
